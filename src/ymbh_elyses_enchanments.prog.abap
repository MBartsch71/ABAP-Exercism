REPORT ymbh_elyses_enchanments.

CLASS lcl_elyses_enchantments DEFINITION.
...
CLASS lcl_elyses_enchantments IMPLEMENTATION.
  ...
  METHOD insert_item_at_top.
    result = stack.
    result = VALUE #( BASE result ( new_card ) ).
  ENDMETHOD.
  ...
  METHOD insert_item_at_bottom.
    result = VALUE #( ( new_card ) ).
    result = VALUE #( BASE result ( LINES OF stack ) ).
  ENDMETHOD.
...
CLASS lth_stack_tester DEFINITION FOR TESTING
  ...
  PROTECTED SECTION.
    DATA cut TYPE REF TO lcl_elyses_enchantments.

    METHODS stack IMPORTING cards         TYPE string
                  RETURNING VALUE(result) TYPE lcl_elyses_enchantments=>ty_stack.
